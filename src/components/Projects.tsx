        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
          {projects.map((project, index) => (
            <motion.div 
              ...
              className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:bg-slate-800/50 transition-colors flex flex-col group relative"
            >
              <div className="overflow-hidden bg-slate-800 aspect-video relative">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                  <h3 className="text-sm font-bold text-white drop-shadow-md">{project.title}</h3>
                  <ExternalLink size={14} className="text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              ...
              <div className="p-4 flex-1 flex flex-col">
                <p className="text-xs text-slate-400 mb-4 flex-1 leading-relaxed">{project.description}</p>
                <div className="flex gap-2 flex-wrap">
                  {project.tags.map((tag, i) => (
                     <span key={i} className="px-2 py-1 text-[9px] font-mono rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase">
                       {tag}
                     </span>
                  ))}
                </div>
