    ...
    <section id="education" className="bg-[#1A1D24] border border-slate-800 rounded-2xl p-6">
       ...
       {loading ? (
         <div className="animate-pulse space-y-4">
           <div><div className="h-4 bg-slate-800 rounded w-3/4 mb-2"></div><div className="h-3 bg-slate-800 rounded w-1/2"></div></div>
           <div><div className="h-4 bg-slate-800 rounded w-3/4 mb-2"></div><div className="h-3 bg-slate-800 rounded w-1/2"></div></div>
         </div>
       ) : (
         <div className="space-y-6">
            {educationItems.map((item, index) => (
              <motion.div key={item.id || index} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                ...
                <p className={`text-xs mt-1 ${item.institution.toLowerCase().includes('andamento') ? 'text-emerald-400 font-mono uppercase tracking-tighter' : 'text-slate-400'}`}>
                  {item.institution}
                </p>
